...
$(document).on('click', '#savebtn', function (event) {
  ...
  fl.append('file_name', $('#ajax_image')[0].files[0].name);
  ...
  if (fd == '') {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        for(i in data.client_data){

          var add_img = `<tr>
                <td>${data.client_data[i].id}</td>
                <td>${data.client_data[i].user_name}</td>
                <td class="img_style"> <img src="${data.client_data[i].image}" alt=""> </td>
            </tr>`
        
        }
        
        $('#tbody').append(add_img)
        $('#ajax_image').val('')
        $('#username').val('')
